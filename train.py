...
def xatlas_uvmap(glctx, geometry, mat, FLAGS):
    ...
    kd_avg = torch.sum(torch.sum(torch.sum(kd * mask, dim=0), dim=0), dim=0) / torch.sum(
        torch.sum(torch.sum(mask, dim=0), dim=0), dim=0
    )
    kd = util.dilate(kd, kd_avg[None, None, None, :], mask, 7)

    ks_avg = torch.sum(torch.sum(torch.sum(ks * mask, dim=0), dim=0), dim=0) / torch.sum(
        torch.sum(torch.sum(mask, dim=0), dim=0), dim=0
    )
...
def initial_guess_material(geometry, mlp, FLAGS, init_mat=None):
    ...
    if mlp:
        ...
        if init_mat is None:
            ...
            kd_init = (
                torch.ones(size=FLAGS.texture_res + [num_channels], device="cuda")
        ...
        mat = {"kd": kd_map_opt, "ks": ks_map_opt, "normal": normal_map_opt}
    ...
    with open(os.path.join(out_dir, "metrics.txt"), "w") as fout:
        ...
        for it, target in enumerate(dataloader_validate):
            ...
            result_image, result_dict = validate_itr(
                glctx, target, dataset_validate.getMesh(), geometry, opt_material, lgt, FLAGS, denoiser
            )
...
def optimize_mesh(
    ...
    learning_rate_lgt = (
        learning_rate[2] if isinstance(learning_rate, list) or isinstance(learning_rate, tuple) else learning_rate * 3.0
    )
...
if __name__ == "__main__":
    ...
    if os.path.splitext(FLAGS.ref_mesh)[1] == ".obj":
        ...
        elif os.path.isfile(os.path.join(FLAGS.ref_mesh, "transforms_train.json")) and not os.path.isfile(
            os.path.join(FLAGS.ref_mesh, "intrinsics.txt")
        ):
    ...
    if FLAGS.base_mesh is None:
        ...
        )  # Use warmup to avoid nasty Adam spikes
