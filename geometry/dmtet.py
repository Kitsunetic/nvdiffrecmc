...
def marching_tets(pos_nx3, sdf_n, tet_fx4):
    ...
    faces = torch.cat(
        (
            torch.gather(
                input=idx_map[num_triangles == 1], dim=1, index=triangle_table[tetindex[num_triangles == 1]][:, :3]
            ).reshape(-1, 3),
            torch.gather(
                input=idx_map[num_triangles == 2], dim=1, index=triangle_table[tetindex[num_triangles == 2]][:, :6]
            ).reshape(-1, 3),
        ),
        dim=0,
    )
...
class DMTetGeometry:
    ...
    def tick(self, glctx, target, lgt, opt_material, loss_fn, iteration, FLAGS, denoiser):
        ...
        if denoiser is not None:
            denoiser.set_influence(shadow_ramp)
        ...
        reg_loss += regularizer.shading_loss(
            buffers["diffuse_light"], buffers["specular_light"], color_ref, FLAGS.lambda_diffuse, FLAGS.lambda_specular
        )
