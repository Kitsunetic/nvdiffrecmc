...
def dilate(x, x_avg, mask, N):
    def _gaussian():
        ...
        gaussian_kernel = (0.5 * np.pi * variance) * torch.exp(-torch.sum(xy_grid**2.0, dim=-1) / (2 * variance))
        return gaussian_kernel / torch.sum(gaussian_kernel)

    def _w(c, cN):
        return torch.stack(
            list(_gaussian() if i == c else torch.zeros(N, N, dtype=torch.float32, device="cuda") for i in range(cN)), dim=0
        )
