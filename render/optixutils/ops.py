...
if _plugin is None:
    ...
    # List of sources.
    source_files = ["c_src/denoising.cu", "c_src/optix_wrapper.cpp", "c_src/torch_bindings.cpp"]
    ...
    def forward(
        ...
        if n_samples_x not in _optix_env_shade_func._random_perm:
            ...
            _optix_env_shade_func._random_perm[n_samples_x] = torch.argsort(
                torch.rand(32768, n_samples_x * n_samples_x, device="cuda"), dim=-1
            ).int()

        diff, spec = _plugin.env_shade_fwd(
...
def optix_env_shade(
    ...
    return _optix_env_shade_func.apply(
        optix_ctx,
        mask,
        ro,
        gb_pos,
