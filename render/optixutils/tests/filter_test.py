        ...
        for y in range(-self.N, self.N + 1):
            for x in range(-self.N, self.N + 1):
                ty, tx = torch.meshgrid(
                    torch.arange(0, input.shape[1], dtype=torch.float32, device="cuda"),
                    torch.arange(0, input.shape[2], dtype=torch.float32, device="cuda"),
                )
                tx = tx[None, ..., None] + x
                ty = ty[None, ..., None] + y
                ...
                with torch.no_grad():
                    ...
                    w_depth = torch.exp(
                        -(torch.abs(zdz_tap[..., 0:1] - zdz[..., 0:1]) / torch.clamp(zdz[..., 1:2] * dist, min=eps))
                    )  # From SVGF

                    w = w_xy * w_normal * w_depth
                    w = torch.where(
                        (tx >= 0) & (tx < input.shape[2]) & (ty >= 0) & (ty < input.shape[1]), w, torch.zeros_like(w)
                    )
